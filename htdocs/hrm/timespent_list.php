...
else if(GETPOST('project_ref', 'alpha')) {
...
if($action == 'confirm_generateinvoice') {
    ...
    if(! ($projectstatic->thirdparty->id > 0)) {
        ...
        if(! $error) {
            ...
            else if($generateinvoicemode == 'onelineperperiod') {    // One line for each time spent line
                $arrayoftasks = [];
            ...
            else if($generateinvoicemode == 'onelinepertask') {    // One line for each different task
                $arrayoftasks = [];
                foreach($toselect as $key => $value) {
                    // Get userid, timepent
...
if(($id > 0 || ! empty($ref)) || $projectidforalltimes > 0 || $allprojectforuser > 0) {
    ...
    if($projectstatic->id > 0 || $allprojectforuser > 0) {
        ...
        $arrayfields['valuebilled'] = [
            'label' => $langs->trans("Billed"),
            'checked' => 1,
            'enabled' => (((getDolGlobalInt('PROJECT_HIDE_TASKS') || ! getDolGlobalInt('PROJECT_BILL_TIME_SPENT')) ? 0 : 1) && $projectstatic->usage_bill_time)
        ];
        // Extra fields
        ...
        else if($action == 'splitline') {
            print '<input type="hidden" name="action" value="updatesplitline">';
        }
        else if($action == 'createtime' && $user->rights->projet->time) {
            print '<input type="hidden" name="action" value="addtimespent">';
        }
        else if($massaction == 'generateinvoice' && $user->hasRight('facture', 'creer')) {
            print '<input type="hidden" name="action" value="confirm_generateinvoice">';
        }
        else if($massaction == 'generateinter' && $user->hasRight('ficheinter', 'creer')) {
        ...
        else if($massaction == 'generateinter') {
            // Form to convert time spent into invoice
        ...
        if($num >= 0) {
            ...
            $title = $langs->trans("TimeSpentList");
        ...
        if($action == 'createtime' && $user->hasRight('projet', 'time')) {
            ...
            if(empty($conf->global->PROJECT_HIDE_TASKS) && ! empty($conf->global->PROJECT_BILL_TIME_SPENT)) {
                ...
                if(isModEnabled("service") && ! empty($projectstatic->thirdparty) && $projectstatic->thirdparty->id > 0 && $projectstatic->usage_bill_time) {
                    ...
                    print $form->select_produits('', 'fk_product', '1', 0, $projectstatic->thirdparty->price_level, 1, 2, '', 1, [], $projectstatic->thirdparty->id, 'None', 0, 'maxwidth150', 0, '', null, 1);
                    print '</td>';
                }
            }
        ...
        foreach($tasks as $task_time) {
            ...
            if(getDolGlobalString('MAIN_CHECKBOX_LEFT_COLUMN')) {
                ...
                else if($user->hasRight('projet', 'time') || $user->hasRight('projet', 'all', 'creer')) {     // Read project and enter time consumed on assigned tasks
            ...
            else if($action == 'editline' && GETPOST('lineid', 'int') == $task_time->rowid) {
            ...
            if(! empty($arrayfields['t.fk_product']['checked'])) {
                ...
                if($action == 'editline' && $_GET['lineid'] == $task_time->rowid) {
                    $form->select_produits($task_time->fk_product, 'fk_product', '1', 0, $projectstatic->thirdparty->price_level, 1, 2, '', 0, [], $projectstatic->thirdparty->id, 'None', 0, 'maxwidth500');
                }
                else if(! empty($task_time->fk_product)) {
            ...
            if($action == 'splitline' && GETPOST('lineid', 'int') == $task_time->rowid) {
                ...
                else if($action == 'splitline' && GETPOST('lineid', 'int') == $task_time->rowid) {
