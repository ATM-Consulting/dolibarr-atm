	...
	public function add_sousproduit($id_pere, $id_fils, $qty, $incdec = 1, $optional=0)
	{
		...
		if (!$this->db->query($sql)) {
			...
			if ($resql > 0) {
				...
				$sql = "INSERT INTO ".$this->db->prefix()."product_association(fk_product_pere,fk_product_fils,qty,incdec,optional,rang)";
				$sql .= " VALUES (".((int) $id_pere).", ".((int) $id_fils).", ".price2num($qty, 'MS').", ".price2num($incdec, 'MS').", ".((int) $optional).", ".((int) $rank).")";
				if (! $this->db->query($sql)) {
					dol_print_error($this->db);
					return -1;
				} else {
