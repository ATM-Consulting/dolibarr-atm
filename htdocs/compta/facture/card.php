...
if (empty($reshook)) {
	...
	if ($action == 'confirm_clone' && $confirm == 'yes' && $permissiontoadd) {
		...
		$facture_qty_requirement = !empty($conf->global->FACTURE_ENABLE_NEGATIVE_QTY) ? ($qty >= 0 || $qty <= 0) : $qty >= 0;
		...
		if (!$error && $facture_qty_requirement && (!empty($product_desc) || (!empty($idprod) && $idprod > 0))) {
			$ret = $object->fetch($id);
			if ($ret < 0) {
				dol_print_error($db, $object->error);
				exit();
...
if ($action == 'create') {
	...
	$morehtmlref .= $form->editfieldval("RefCustomer", 'ref_client', $object->ref_client, $object, $usercancreate, 'string'.(isset($conf->global->THIRDPARTY_REF_INPUT_SIZE) ? ':'.$conf->global->THIRDPARTY_REF_INPUT_SIZE : ''), '', null, null, '', 1);	// InfraS change
	// Thirdparty
	...
	if ($action != 'prerelance' && $action != 'presend' && $action != 'valid' && $action != 'editline') {
		...
		if (empty($reshook)) {
			...
			if ($object->statut == Facture::STATUS_VALIDATED) {
				...
				if ($ventilExportCompta == 0) {
					if (empty($conf->global->INVOICE_CAN_NEVER_BE_EDITED)) {
						...
						print '<span class="butActionRefused classfortooltip" title="' . $langs->trans("DisabledBecauseEditedAnInvoiceIsProhibited") . '">' . $langs->trans('Modify') . '</span>';
					}
				} else {
	...
	if ($action != 'prerelance' && $action != 'presend') {
		...
		$linktoelem = $form->showLinkToObjectBlock($object, null, array());	// InfraS change
