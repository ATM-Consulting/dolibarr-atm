...
class Account extends CommonObject
{
	 ...
	 */
    public function addline($date, $oper, $label, $amount, $num_chq, $categorie, User $user, $emetteur = '', $banque = '', $accountancycode = '', $datev = null,$fromreglement = null)
	{
		// Deprecation warning
		if (is_numeric($oper)) {
		...
		if ($fromreglement) {
			$accline->fromreglement = $fromreglement;
		}
		if ($num_chq) {
...
class AccountLine extends CommonObject
{
    ...
    public function fetch($rowid, $ref = '', $num = '')
	{
		...
		$sql .= " b.fk_bordereau, b.banque, b.emetteur,b.fromreglement,";
		...
		if ($result)
		{
			...
			if ($obj)
			{
				...
				$this->fromreglement = $obj->fromreglement;
	...
	public function insert()
	{
		...
		$sql .= ", ".($this->fromreglement ? "'".$this->db->escape($this->fromreglement)."'" : "null")."";
		$sql .= ")";
