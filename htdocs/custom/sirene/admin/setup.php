...
if (!empty($conf->use_javascript_ajax)) {
	print ajax_constantonoff('SIRENE_DEBUG', '', $conf->entity);
} else {
...
if (empty($conf->global->SIRENE_PROCESSING_TOKEN)) {
	...
	dolibarr_set_const($db, "SIRENE_PROCESSING_TOKEN", 0, 'chaine', 0, '', $conf->entity);
} else {
	print '<a href="' . $_SERVER['PHP_SELF'] . '?action=del_SIRENE_PROCESSING_TOKEN">' . img_picto($langs->trans("Enabled"), 'switch_on') . '</a>';
	dolibarr_set_const($db, "SIRENE_PROCESSING_TOKEN", 1, 'chaine', 0, '', $conf->entity);
}
print '</td></tr>' . "\n";
*/
