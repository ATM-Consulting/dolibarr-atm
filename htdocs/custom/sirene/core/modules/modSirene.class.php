...
class modSirene extends DolibarrModules
{
	...
	public function __construct($db)
	{
		...
		$this->const = array(
			...
			3 => array('SIRENE_API_URL', 'chaine', 'https://api.insee.fr/entreprises/sirene/V3', '', 0, 'current', 0),
			4 => array('SIRENE_VERIFICATION_SIRET_URL', 'chaine',
					   'https://annuaire-entreprises.data.gouv.fr/entreprise/', '', 0, 'current', 0),
			5 => array('SIRENE_MAIL_TO_SEND', 'chaine', '', '', 0, 'current', 0),
			//6 => array('SIRENE_PROCESSING_TOKEN', 'chaine', 1, '', 0, 'current', 0),
			7 => array('MODULE_SIRENE_VERSION', 'chaine', $this->version, '', 0, 'allentities', 0)
		);
		...
		$this->dictionaries = array(
			'langs'          => 'sirene@sirene',
			'tabname'        => array(MAIN_DB_PREFIX . "c_codenaf"),
			// List of tables we want to see into dictionary editor
			'tablib'         => array("SireneCodeNafDictionaryTable"),// Label of tables
			//'tabsql'=>array('DELETE FROM '.MAIN_DB_PREFIX.' FROM c_sirene_country'),	// Request to select fields
			'tabsql'         => array('SELECT f.rowid as rowid, f.code, f.label, f.active FROM ' . MAIN_DB_PREFIX . 'c_codenaf as f'),
			...
			'tabcond'        => array($conf->sirene->enabled)
	...
	{
		global $sirene, $conf, $langs, $hookmanager, $extrafields, $parameters;
		...
		$result = $extrafields->addExtraField('sirene_status', $langs->trans('SireneCompanyStatus'), 'boolean', 100, '', 'societe', 0, 0, '', null, 0, '', 2, 0, '', '', 'sirene@sirene', '1');
		$result = $extrafields->addExtraField('sirene_update_date', $langs->trans("SireneCompanyUpdateDate"), 'datetime', 101, '', 'societe', 0, 0, '', null, 0, '', 2, 0, '', '', 'sirene@sirene', '1');

		$result = $extrafields->update('sirene_status', $langs->trans('SireneCompanyStatus'), 'boolean', '', 'societe', 0, 0, 100, null, 0, '', 2, 0, '', '', '', '0');
		$result = $extrafields->update('sirene_update_date', $langs->trans('SireneCompanyUpdateDate'), 'datetime', '', 'societe', 0, 0, 101, null, 0, '', 2, 0, '', '', '');
		...
		if (empty($reshook) && !empty($extrafields->attribute_label)) {
			print $extrafields->addExtraField($result);
		}
		if (!$res) {
			return 0;
		}
