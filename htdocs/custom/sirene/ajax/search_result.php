...
if (!empty($sirene_company_name) || !empty($sirene_siret) || !empty($sirene_siren) || !empty($sirene_naf) || !empty($sirene_zipcode)) {
	...
	if ($result < 0) {
		...
		$return['html'] .= '<br>'.$sirene->errorsToString('<br>');
	} elseif ($result > 0) {
		...
		//    $staff = $sirene->companies_results[0]['staff'];
		//    $legal_category = $sirene->companies_results[0]['legalcategory'];
		//    $legal_category_level_2 = $sirene->companies_results[0]['legalcategory_level_2'];
		...
		if ($result < 0) {
			...
			foreach ($sirene->companies_results as $key => $company_infos) {
				...
				$table_choices .= '<input type="hidden" id="sirene_staff_' . $idLine . '" value="' . $company_infos['staff'] . '" />';
				$table_choices .= '<input type="hidden" id="sirene_legalcategory_' . $idLine . '" value="' . $company_infos['legalcategory'] . '" />';
				$table_choices .= '<input type="hidden" id="sirene_staff_label_' . $idLine . '" value="' . $company_infos['staff_label'] . '" />';
				$table_choices .= '<input type="hidden" id="sirene_legalcategory_id_' . $idLine . '" value="' . $company_infos['legalcategory_id'] . '" />';
			...
			$outJSAjax .= '             sirene_staff: jQuery("#sirene_staff_" + idLineSelected).val(),';
			$outJSAjax .= '             sirene_legalcategory: jQuery("#sirene_legalcategory_" + idLineSelected).val(),';
			$outJSAjax .= '             sirene_staff_label: jQuery("#sirene_staff_label_" + idLineSelected).val(),';
			$outJSAjax .= '             sirene_legalcategory_id: jQuery("#sirene_legalcategory_id_" + idLineSelected).val(),';
