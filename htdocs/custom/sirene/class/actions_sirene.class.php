...
class ActionsSirene
{
	...
	public function doActions($parameters, &$object, &$action, $hookmanager)
	{
		...
		//        //récupération de l'effectif dans dolibarr via Societe
		...
		if (in_array('thirdpartycard', $context)) {
			...
			if (GETPOST('sirene_search') != '') {
				...
				if ($confirm == 'yes') {
					...
					if ($result < 0) {
						...
						$_GET['effectif_id'] = $company_infos['staff'];  // effectif
						$_GET['forme_juridique_code'] = $company_infos['legalcategory'];  // type d'entité légale niveau 2
					}
				}
				...
				if ($confirm == 'yes') {
					...
					if ($result < 0) {
						...
						$_GET['country_code'] = $company_infos['country_code'];
						$_GET['country_san'] = $company_infos['country_san'];
						$_GET['effectif'] = $company_infos['staff'];  //effectif
						$_GET['forme_juridique'] = $company_infos['legalcategory'];  //type d'entité légale niveau 2
					}
				}
				...
				$correspondance = array(
					...
					'staff'            => 'effectif_id', // effectif
					'legalcategory'    => 'forme_juridique_code' // type d'entité légale niveau 2
				);
			...
			} elseif ($action == 'confirm_sir_data' && $confirm == 'yes' && $user->rights->societe->creer) {
				...
				//recherche par rapport au nom du tiers et du siren
				if ($result < 0) {
					setEventMessages($langs->trans('SireneErrorWhileGetCompanies'), $sirene->errors, 'errors');
					return -1;
				} elseif (empty($sirene->companies_results)) {
			...
			} elseif ($action == 'confirm_sirene_update' && $confirm == 'yes' && $user->rights->societe->creer) {
				// Definition des lignes
				$correspondance = array(
					'company_name'     => 'name',
					'address'          => 'address',
	...
	public function formObjectOptions($parameters, &$object, &$action, $hookmanager)
	{
		...
		if (in_array('thirdpartycard', $context)) {
			if ($action != 'create' && $action != 'edit') {
				...
				$sirene_siren = str_replace(' ', '', $sirene_siren);

				print '<td>' . $langs->trans("SireneSiren") . ' : <input type="text" name="sirene_siren" value="' . $sirene_siren . '"></td>' . "\n";

				$sirene_siret = GETPOST('sirene_siret', 'alpha');
				$sirene_siret = str_replace(' ', '', $sirene_siret);

				print '<td>' . $langs->trans("SireneSiret") . ' : <input type="text" name="sirene_siret" value="' . $sirene_siret . '"></td>' . "\n";
				...
				if (!empty($object->idprof2)) {
					...
					if ($act == 'sir_data' && $user->rights->societe->creer && !empty($object->idprof2)) {
						...
						//	dolibarr_set_const($this->db, "SIRENE_PROCESSING_TOKEN", 0, 'chaine', 0, '', $conf->entity);
							...
							$correspondance = array(
								'company_name'         => array('title'    => $langs->trans("SireneCompanyName"),
																'dolibarr' => $object->name,
																'sirene'   => $company_infos['company_name']),
								'address'              => array('title'    => $langs->trans("Address"),
																'dolibarr' => $object->address,
																'sirene'   => $company_infos['address']),
								'zipcode'              => array('title'    => $langs->trans("CompanyZip"),
																'dolibarr' => $object->zip,
																'sirene'   => $company_infos['zipcode']),
								'town'                 => array('title'    => $langs->trans("SireneTown"),
																'dolibarr' => $object->town,
																'sirene'   => $company_infos['town']),
								//'state_id' => array('title' => $langs->trans("SireneSateID"), 'dolibarr' => $object->state_id, 'sirene' => $company_infos['state_id']),
								'siren'                => array('title'    => $langs->trans("SireneSiren"),
																'dolibarr' => $object->idprof1,
																'sirene'   => $company_infos['siren']),
								'siret'                => array('title'    => $langs->trans("SireneSiret"),
																'dolibarr' => $object->idprof2,
																'sirene'   => $company_infos['siret']),
								'codenaf_san'          => array('title'    => $langs->trans("SireneNaf"),
																'dolibarr' => $object->idprof3,
																'sirene'   => $company_infos['codenaf_san']),
								'sirene_tva_intra'     => array('title'    => $langs->trans("SireneTvaIntra"),
																'dolibarr' => $object->tva_intra,
																'sirene'   => $company_infos['sirene_tva_intra']),
								'country'              => array('title'    => $langs->trans("SireneCountry"),
																'dolibarr' => $object->country,
																'sirene'   => $company_infos['country']),
								//c'est le country ID qui change le pays
								'country_id'           => array('title'    => $langs->trans("SireneCountryISO"),
																'dolibarr' => $object->country_id,
																'sirene'   => $company_infos['country_id']),
								...
								'staff_label'          => array('title'    => $langs->trans("SireneStaff"),
																'dolibarr' => $object->effectif,
																'sirene'   => $company_infos['staff_label']),
								//type d'entité légale
								'legalcategory_id'     => array('title'    => $langs->trans("SireneJuridicalStatusLevel2"),
																'dolibarr' => $object->forme_juridique,
																'sirene'   => $company_infos['legalcategory_id']),
								//type d'entité légale

								'staff'         => array('title'    => $langs->trans("SireneStaff"),
														 'dolibarr' => $object->effectif_id,
														 'sirene'   => $company_infos['staff']), //type d'entité légale
								'legalcategory' => array('title'    => $langs->trans("SireneJuridicalStatusLevel2"),
														 'dolibarr' => $object->forme_juridique_code,
														 'sirene'   => $company_infos['legalcategory']), //effectif
							...
							$table_choices .= '<td><span class="liste_titre" style="font-size: 12px; font-size: large; title="' . $langs->trans("SireneFieldNameDesc") . '">' . $langs->trans("SireneFieldName") . '</span><td>' . "\n";
							$table_choices .= '<td><span class="liste_titre" style="font-size: 12px; font-size: large;" title="' . $langs->trans("SireneDataDolibarrDesc") . '">' . $langs->trans("SireneDataDolibarr") . '</span><td>' . "\n";
							$table_choices .= '<td><span class="fas fa-caret-left marginleftonlyshort valignmiddle"  style="font-size: 28px" title="' . $langs->trans("SireneInputUpdateDesc") . '"></span><td>' . "\n";
							$table_choices .= '<td><input type="checkbox" id="checkallactions" value="1"><td>' . "\n";
							$table_choices .= '<td><span class="liste_titre" style="font-size: 12px; font-size: large;" title="' . $langs->trans("SireneDataSireneDesc") . '">' . $langs->trans("SireneDataSirene") . '</span><td>' . "\n";
							$table_choices .= '</tr>' . "\n";
...
SCRIPT;
							...
							foreach ($correspondance as $key => $infos) {
								if ($key == 'country_id' || $key == 'staff' || $key == 'legalcategory') {
							...
							if ($company_infos['status'] == "F") {
								//si le siret est celui d'un tiers fermé
								print $form->formconfirm($_SERVER["PHP_SELF"] . '?socid=' . $object->id, $langs->trans('PopUpChangeBoxSirene'), $langs->trans('PopUpAlertClosedThirdparty'), "confirm_sir_data", $formquestion, 'no', 1, 600, '80%');
							} else {
					...
					if ($user->rights->societe->creer) {
						...
						if ($act == 'sir_import' && $user->rights->societe->creer) {
							...
							//	dolibarr_set_const($this->db, "SIRENE_PROCESSING_TOKEN", 0, 'chaine', 0, '', $conf->entity);
								...
								$outJSAjax .= '             sirene_only_open: jQuery("#sirene_only_open").is(":checked") ? 1 : 0,';
