...
class Sirene
{
	...
	public function cronSirene()
	{
		...
		if (empty($conf->global->SIRENE_PROCESSING_SYNCHRONIZATION)) {
			...
			if ($result < 0) {
				...
				$test = $this->legalcategory_level_2;
			}
			...
			$correspondance = array(
				...
				'codenaf_san'        => 'idprof3',
				'tva_intra'          => 'tva_intra',
				'staff'              => 'effectif', //type d'entité légale
				'legalcategory'      => 'forme_juridique',  //effectif
				//'legalcategory_level_2'=> $test,
			);
	 ...
	 * @return  int                             <0 if KO, >0 if OK
	 */
	public function getCompanies($company_name, $siren, $siret, $naf, $zipcode, $nombre, $only_open = 0)
	{
		global $conf, $langs;
		...
		try {
			$response = $this->client->post('siret', [
				'headers'                              => ['Authorization' => 'Bearer ' . $conf->global->SIRENE_API_BEARER_KEY],
				GuzzleHttp\RequestOptions::FORM_PARAMS => ['q' => implode(' AND ', $filter), 'nombre' => $nombre]
			]);
			...
			foreach ($results as $company_infos) {
				...
				$establishment = $company_infos['periodesEtablissement'][0]['enseigne1Etablissement'];
				$establishmentOrNot = is_null($establishment);
				if ($establishmentOrNot == 0) {
					$company_name = $company_name_all = $company_infos['periodesEtablissement'][0]['enseigne1Etablissement'];
					if (!empty($company_name)) {
						$company_name = $company_infos['periodesEtablissement'][0]['enseigne1Etablissement'];
						$company_name_alias = $company_infos['periodesEtablissement'][0]['enseigne1Etablissement'];
						if (empty($company_name_alias)) $company_name_alias = $company_infos['periodesEtablissement'][0]['enseigne1Etablissement'];

						$private = 0;
					}
				} elseif ($establishmentOrNot == 1) {
				...
				$staff = $company_infos['uniteLegale']['trancheEffectifsUniteLegale'];
				$legalcategory = $company_infos['uniteLegale']['categorieJuridiqueUniteLegale'];
				$legalcategory_level_2 = strval($legalcategory);
				$legalcategory_level_2 = substr($legalcategory_level_2, 0, 2);
				...
				$this->companies_results[] = array(
					...
					'legalcategory_level_2' => $legalcategory_level_2
			...
			if (isset($response)) {
				...
				if (!empty($conf->global->SIRENE_DEBUG)) {
					if (isset($request)) $this->errors[] = $this->requestToString($request);
					if (isset($response)) $this->errors[] = $this->responseToString($response);
					else $this->errors[] = '<pre>' . dol_nl2br((string) $e) . '</pre>';
				} else {
					$this->errors[] = '<b>' . $langs->trans('SireneResponseCode') . ': </b>' . $response->getStatusCode() .
						'<br><b>' . $langs->trans('SireneResponseReasonPhrase') . ': </b>' . $response->getReasonPhrase() . '<br>';
				}
			} else {
				if (!empty($conf->global->SIRENE_DEBUG)) {
