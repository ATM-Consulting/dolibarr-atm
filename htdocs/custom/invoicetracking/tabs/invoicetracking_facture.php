...
/**
 *      \file       htdocs/comm/facture/info.php
 *      \ingroup    facture
...
llxHeader('', $langs->trans('invoicetracking'), 'EN:Commercial_Proposals|FR:Proposition_commerciale|ES:Presupuestos');
...
dol_fiche_head($head, 'invoicetracking', $langs->trans('Invoice'), 0, 'bill');
...
print_liste_field_titre($langs->trans('Invoice'), $_SERVER['PHP_SELF'], 't.fk_facture', '', $param, '', $sortfield, $sortorder);
    ...
    if ($num) {
        while ($i < $num) {
            ...
            if ($obj) {
                ...
                print '</td><td>';
                if ($version >= 6) {
                    print '<a href="' . DOL_URL_ROOT . '/compta/facture/card.php?id=' . $obj->fk_facture . '">' . img_object($langs->trans("ShowPropal"), "bill") . $p->ref . '</a>';
                } else {
                    print '<a href="' . DOL_URL_ROOT . '/compta/facture.php?facid=' . $obj->fk_facture . '">' . img_object($langs->trans("ShowPropal"), "bill") . $p->ref . '</a>';
                }
...
if ($object->statut == 1) {
    ...
    print '<div class="tabsAction">
        <div class="inline-block divButAction">
            <a href="' . dol_buildpath("/invoicetracking/index.php?action=add&facid=$id", 1) . '" class="butAction">' . $langs->trans('Add') . '</a>
        </div>
        </div>';
}
