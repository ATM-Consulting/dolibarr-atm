	...
	if ($tabname[$id])
	{
		...
		foreach ($fieldlist as $field => $value)
		{
			...
			if ($fieldlist[$field] == 'type') {
				if ($tabname[$id] == MAIN_DB_PREFIX."c_paiement") $valuetoshow = $form->textwithtooltip($langs->trans("Type"), $langs->trans((!empty($conf->global->INFRASPLUS_PDF_USE_PAY_SPEC) ? "InfraSPlusDictListPaiements" : "TypePaymentDesc")), 2, 1, img_help(1, ''));	// InfraS change '"TypePaymentDesc"' to '(!empty($conf->global->INFRASPLUS_PDF_USE_PAY_SPEC) ? "InfraSPlusDictListPaiements" : "TypePaymentDesc")'
					...
					if (empty($reshook))
					{
						...
						foreach ($fieldlist as $field => $value)
						{
							...
							if ($value == 'private')
							{
                            	...
                            	$payment_type_list = array(0=>$langs->trans('PaymentTypeCustomer'), 1=>$langs->trans('PaymentTypeSupplier'), 2=>$langs->trans('PaymentTypeBoth'), 3=>$langs->trans('InfraSPlusDictPaymentTypeSpe'));	// InfraS add ", 3=>$langs->trans('InfraSPlusDictPaymentTypeSpe')"
...
function fieldList($fieldlist, $obj = '', $tabname = '', $context = '')
{
	...
	foreach ($fieldlist as $field => $value)
	{
		...
		{
			print '<td>';
			$select_list = array(0=>$langs->trans('PaymentTypeCustomer'), 1=>$langs->trans('PaymentTypeSupplier'), 2=>$langs->trans('PaymentTypeBoth'), 3=>$langs->trans('InfraSPlusDictPaymentTypeSpe'));	// InfraS add ", 3=>$langs->trans('InfraSPlusDictPaymentTypeSpe')"
