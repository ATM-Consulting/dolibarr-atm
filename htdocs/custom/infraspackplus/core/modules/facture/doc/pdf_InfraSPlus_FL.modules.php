	...
	class pdf_InfraSPlus_FL extends ModelePDFFactures
	{
		...
		********************************************/
		public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
		{
			...
			$filesufixe												= ! $this->multi_files || ($this->defaulttemplate && $this->defaulttemplate == 'InfraSPlus_FL') ? '' : '_FL';
			...
			if ($baseDir) {
				...
				if (file_exists($dir)) {
					...
					for ($i = 0 ; $i < $nblignes ; $i++) {
						...
						if ($this->use_multicurrency)	$tvaligne		= doubleval($this->sign * $object->lines[$i]->multicurrency_total_tva);
						else							$tvaligne		= doubleval($this->sign * $object->lines[$i]->total_tva);
		...
		********************************************/
		protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
		{
			...
			if ($showaddress) {
				...
				$addresses		= array();
				$addresses		= pdf_InfraSPlus_getAddresses($object, $outputlangs, $arrayidcontact, $this->adr, $this->adrlivr, $this->emetteur, 1, '', $this->adrfact);
		...
		********************************************/
		protected function _tableau_info(&$pdf, $object, $posy, $outputlangs, $calculseul = 0)
		{
			...
			if ($this->shippings['fk_shipping_method'] > 0) {
				...
				$pdf->SetFont('', '', $default_font_size - 2);
				$label			= '';
				$code			= $outputlangs->getLabelFromKey($db, $this->shippings['fk_shipping_method'], 'c_shipment_mode', 'rowid', 'code');	// Get code using getLabelFromKey
		...
		********************************************/
		protected function _tableau_tot(&$pdf, $object, $posy, $outputlangs, $calculseul = 0)
		{
			...
			if (empty($this->only_ttc)) {
				...
				$pdf->MultiCell($larg_col1total, $tabtot_hl, $outputlangs->transnoentities(!empty($this->situationinvoice) && count($this->previnvoices) ? 'PDFInfrasPlusCumulSituation' : 'TotalHTShort').(!empty($this->hasEcoTaxes) ? ' '.$this->hasEcoTaxes : ''), '', 'L', 0, 1, $posxtabtotal, $posytabtot + (($tabtot_hl + $this->bgLineW) * $index), true, 0, 0, false, 0, 'M', false);
			...
			if (empty($this->only_ht)) {
				...
				if ($this->situationinvoice && count($this->previnvoices)) {
					foreach ($this->previnvoices as $invoice) {
						$index++;
						$pdf->SetTextColor($this->bodytxtcolor[0], $this->bodytxtcolor[1], $this->bodytxtcolor[2]);
						$pdf->SetAlpha($this->alpha);
					...
					$actualref	= $outputlangs->transnoentities('InvoiceSituation').$outputlangs->convToOutputCharset(" n°".$object->situation_counter);
					$pdf->MultiCell($larg_col1total, $tabtot_hl, $actualref, '', 'L', 0, 1, $posxtabtotal, $posytabtot + (($tabtot_hl + $this->bgLineW) * $index), true, 0, 0, false, 0, 'M', false);
					$pdf->MultiCell($larg_col2total, $tabtot_hl, pdf_InfraSPlus_price($object, $actual_ttc, $outputlangs), '', 'R', 0, 1, $posxcol2total, $posytabtot + (($tabtot_hl + $this->bgLineW) * $index), true, 0, 0, false, 0, 'M', false);
				}
			}
			$pdf->SetTextColor($this->bodytxtcolor[0], $this->bodytxtcolor[1], $this->bodytxtcolor[2]);
			// ecoTaxes
