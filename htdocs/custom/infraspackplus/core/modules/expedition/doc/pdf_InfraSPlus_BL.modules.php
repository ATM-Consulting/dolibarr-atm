	...
	class pdf_InfraSPlus_BL extends ModelePdfExpedition
	{
		...
		********************************************/
		public function __construct($db)
		{
			...
			$this->show_efl						= isset($conf->global->INFRASPLUS_PDF_BL_WITH_POS_COLUMN)			? $conf->global->INFRASPLUS_PDF_BL_WITH_POS_COLUMN			: 0;
			$this->exf_prod_pos					= isset($conf->global->INFRASPLUS_PDF_EXF_PROD_POS)					? $conf->global->INFRASPLUS_PDF_EXF_PROD_POS				: '';
			$this->hide_wv						= isset($conf->global->SHIPPING_PDF_HIDE_WEIGHT_AND_VOLUME)			? $conf->global->SHIPPING_PDF_HIDE_WEIGHT_AND_VOLUME		: 0;
			$this->hide_ordered					= isset($conf->global->INFRASPLUS_PDF_HIDE_ORDERED)					? $conf->global->INFRASPLUS_PDF_HIDE_ORDERED				: 0;
			$this->larg_ref						= isset($conf->global->INFRASPLUS_PDF_LARGCOLBL_REF)				? $conf->global->INFRASPLUS_PDF_LARGCOLBL_REF				: 28;
			$this->larg_efl						= isset($conf->global->INFRASPLUS_PDF_LARGCOLBL_EFL)				? $conf->global->INFRASPLUS_PDF_LARGCOLBL_EFL				: 14;
			$this->larg_wv						= isset($conf->global->INFRASPLUS_PDF_LARGCOLBL_WV)					? $conf->global->INFRASPLUS_PDF_LARGCOLBL_WV				: 22;
			$this->larg_unit					= isset($conf->global->INFRASPLUS_PDF_LARGCOLBL_UNIT)				? $conf->global->INFRASPLUS_PDF_LARGCOLBL_UNIT				: 10;
			$this->larg_ordered					= isset($conf->global->INFRASPLUS_PDF_LARGCOLBL_ORDERED)			? $conf->global->INFRASPLUS_PDF_LARGCOLBL_ORDERED			: 10;
			$this->larg_rel						= isset($conf->global->INFRASPLUS_PDF_LARGCOLBL_REL)				? $conf->global->INFRASPLUS_PDF_LARGCOLBL_REL				: 10;
			$this->larg_qty						= isset($conf->global->INFRASPLUS_PDF_LARGCOLBL_QTY)				? $conf->global->INFRASPLUS_PDF_LARGCOLBL_QTY				: 10;
			$this->num_ref						= isset($conf->global->INFRASPLUS_PDF_NUMCOLBL_REF)					? $conf->global->INFRASPLUS_PDF_NUMCOLBL_REF				: 1;
			$this->num_efl						= isset($conf->global->INFRASPLUS_PDF_NUMCOLBL_EFL)					? $conf->global->INFRASPLUS_PDF_NUMCOLBL_EFL				: 2;
			$this->num_desc						= isset($conf->global->INFRASPLUS_PDF_NUMCOLBL_DESC)				? $conf->global->INFRASPLUS_PDF_NUMCOLBL_DESC				: 3;
			$this->num_wv						= isset($conf->global->INFRASPLUS_PDF_NUMCOLBL_WV)					? $conf->global->INFRASPLUS_PDF_NUMCOLBL_WV					: 4;
			$this->num_unit						= isset($conf->global->INFRASPLUS_PDF_NUMCOLBL_UNIT)				? $conf->global->INFRASPLUS_PDF_NUMCOLBL_UNIT				: 5;
			$this->num_ordered					= isset($conf->global->INFRASPLUS_PDF_NUMCOLBL_ORDERED)				? $conf->global->INFRASPLUS_PDF_NUMCOLBL_ORDERED			: 6;
			$this->num_rel						= isset($conf->global->INFRASPLUS_PDF_NUMCOLBL_REL)					? $conf->global->INFRASPLUS_PDF_NUMCOLBL_REL				: 7;
			$this->num_qty						= isset($conf->global->INFRASPLUS_PDF_NUMCOLBL_QTY)					? $conf->global->INFRASPLUS_PDF_NUMCOLBL_QTY				: 8;
			...
			if ($baseDir) {
				...
				if (file_exists($dir)) {
					...
					if (!empty($this->show_rel_col)) {
						...
						$staticCommande->fetch($object->origin_id);
						$staticCommande->loadExpeditions();
					}
					$this->totaux	= array();
					$qty_rel		= array();
					$realpatharray	= array();
					...
					for ($i = 0 ; $i < $nblignes ; $i++) {
						...
						$qty_rel[$i]				= $object->lines[$i]->qty_asked - $staticCommande->expeditions[$object->lines[$i]->fk_origin_line];
						$this->totaux['rel']		+= $qty_rel[$i];
						$isProd						= $objproduct->fetch($object->lines[$i]->fk_product);
						if (!empty($this->show_efl) && $isProd > 0) {
							$extrafieldsprod	= new ExtraFields($db);
							...
							foreach ($extralabelsprod as $key => $label) {
								if ($this->exf_prod_pos && substr($key, 0, strlen($this->exf_prod_pos)) !== $this->exf_prod_pos)	continue;
								$options_key																						= $objproduct->array_options['options_'.$key];
								$this->label_efl																					= $label;
								$value																								= $extrafieldsprod->showOutputField($key, $options_key);
							}
					...
					if(! $this->show_efl)			$this->larg_efl		= 0;
					if($this->hide_wv)				$this->larg_wv		= 0;
					if(! $this->product_use_unit)	$this->larg_unit	= 0;
					if($this->hide_ordered)			$this->larg_ordered	= 0;
					if(! $this->show_rel_col)		$this->larg_rel		= 0;
					$this->larg_desc				= $this->larg_util_cadre - ($this->larg_ref + $this->larg_efl + $this->larg_wv + $this->larg_unit + $this->larg_ordered + $this->larg_rel + $this->larg_qty); // Largeur variable suivant la place restante
					...
					for ($i = 0 ; $i < $nblignes ; $i++) {
						...
						// Position
						if (!empty($this->show_efl))	$pdf->MultiCell($this->tableau['efl']['larg'], $this->heightline, $prod_pos[$i], '', 'C', 0, 1, $this->tableau['efl']['posx'], $curY, true, 0, 0, false, 0, 'M', false);
						// volume & weight
						if (empty($this->hide_wv)) {
							$weighttxt	= '';
							$voltxt		= '';
							if ($object->lines[$i]->fk_product_type == 0) {
								if ($object->lines[$i]->weight)	$weighttxt	= round($object->lines[$i]->weight * $object->lines[$i]->qty_shipped, 2).' '.measuringUnitString(0, 'weight', $object->lines[$i]->weight_units, 1);
								if ($object->lines[$i]->volume)	$voltxt		= round($object->lines[$i]->volume * $object->lines[$i]->qty_shipped, 2).' '.measuringUnitString(0, 'volume', ($object->lines[$i]->volume_units ? $object->lines[$i]->volume_units : 0), 1);
							}	//if ($object->lines[$i]->fk_product_type == 0)
		...
		********************************************/
		protected function _tableau(&$pdf, $object, $tab_top, $tab_height, $outputlangs, $hidetop = 0, $hidebottom = 0, $pagenb)
		{
			...
			if (empty($hidetop) || $pagenb == 1) {
				...
				if (!empty($this->show_efl))		$pdf->MultiCell($this->tableau['efl']['larg'], $this->ht_top_table, $outputlangs->transnoentities($this->label_efl), '', 'C', 0, 1, $this->tableau['efl']['posx'], $tab_top, true, 0, false, true, $this->ht_top_table, 'M', false);
				...
				if (empty($this->hide_wv))			$pdf->MultiCell($this->tableau['wv']['larg'], $this->ht_top_table, $outputlangs->transnoentities('WeightVolShort'), '', 'C', 0, 1, $this->tableau['wv']['posx'], $tab_top, true, 0, false, true, $this->ht_top_table, 'M', false);
		...
		********************************************/
		protected function _tableau_tot(&$pdf, $object, $posy, $outputlangs, $calculseul = 0)
		{
			...
			if (empty($this->hide_wv)) {
				...
				$pdf->SetFont('', 'B', $default_font_size - 1);
				$this->title_bg ? $pdf->SetTextColor($this->txtcolor[0], $this->txtcolor[1], $this->txtcolor[2]) : $pdf->SetTextColor($this->bodytxtcolor[0], $this->bodytxtcolor[1], $this->bodytxtcolor[2]);
				if ($this->tableau['wv']['col'] > 1)	$pdf->MultiCell($larg_col1total, $tabtot_hl, $outputlangs->transnoentities(empty($this->hide_wv) ? 'Total' : ''), '', 'L', 0, 1, $posxtabtotal, $posytabtot + ($tabtot_hl * $index), true, 0, 0, false, 0, 'M', false);
				else									$totalWeighttoshow .= $totalWeighttoshow ? ' (Total)' : '';
				// Total weight
			...
			else	$pdf->MultiCell($larg_col1total, $tabtot_hl, ' ', '', 'L', 0, 1, $posxtabtotal, $posytabtot + ($tabtot_hl * $index), true, 0, 0, false, 0, 'M', false);
			$pdf->SetFont('', '', $default_font_size - 1);
			$pdf->SetTextColor($this->bodytxtcolor[0], $this->bodytxtcolor[1], $this->bodytxtcolor[2]);
