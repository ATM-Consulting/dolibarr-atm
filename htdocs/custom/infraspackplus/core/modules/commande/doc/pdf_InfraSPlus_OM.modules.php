<?php
	...
	************************************************/
	...
	/************************************************
	* 	\file		../infraspackplus/core/modules/commande/doc/pdf_InfraSPlus_OM.modules.php
	* 	\ingroup	InfraS
	...
	************************************************/
	class pdf_InfraSPlus_OM extends ModelePDFCommandes
	{
		public $db;
		public $name;
		public $description;
		...
		********************************************/
		public function __construct($db)
		{
			...
			$this->name							= $langs->trans('PDFInfraSPlusOMName');
			$this->description					= $langs->trans('PDFInfraSPlusOMDescription');
			$this->titlekey						= 'PDFInfraSPlusOMTitle';
		...
		********************************************/
		public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
		{
			...
			$filesufixe						= ! $this->multi_files || ($this->defaulttemplate && $this->defaulttemplate == 'InfraSPlus_OM') ? '' : '_OF';
			...
			if ($baseDir) {
				...
				if (file_exists($dir)) {
					...
					if (! $this->hide_cols)											$this->larg_desc	= $this->larg_util_cadre - ($this->larg_qty + $this->larg_unit + $this->larg_ref); // Largeur variable suivant la place restante
					...
					$pdf->SetTextColor($this->bodytxtcolor[0], $this->bodytxtcolor[1], $this->bodytxtcolor[2]);
					$posyfreetext		= ! $this->free_text_end ? pdf_InfraSPlus_free_text($pdf, $object, $this->formatpage, $this->posxtabtotal, $posyinfo, $outputlangs, $this->emetteur, $this->listfreet, 0, 0, $this->horLineStyle) : $bottomlasttab;
					if ($this->show_sign_area) {
		...
		********************************************/
		protected function _signature_area(&$pdf, $object, $posy, $outputlangs, $calculseul = 0, $freetext = 0)
		{
			...
			if ($freetext)			$pdf->Line($posxsignarea1, $posy, $this->page_largeur - $this->marge_droite, $posy, $this->horLineStyle);
			...
			if ($calculseul) {
				...
				if ($this->e_signing) {
					if ($this->show_2sign_area)	$pdf->addEmptySignatureAppearance($posxsignarea1, $signarea_top + $signarea_hl, $larg_signarea, $this->ht_signarea);
