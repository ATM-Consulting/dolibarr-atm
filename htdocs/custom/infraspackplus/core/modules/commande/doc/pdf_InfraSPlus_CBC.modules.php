<?php
	...
	************************************************/
	...
	/************************************************
	* 	\file		../infraspackplus/core/modules/commande/doc/pdf_InfraSPlus_CBC.modules.php
	* 	\ingroup	InfraS
	...
	class pdf_InfraSPlus_CBC extends ModelePDFCommandes
	{
		...
		********************************************/
		public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
		{
			...
			$filesufixe						= ! $this->multi_files || ($this->defaulttemplate && $this->defaulttemplate == 'InfraSPlus_CBC') ? '' : '_CBC';
				...
				if (file_exists($dir)) {
					...
					for ($i = 0 ; $i < $nblignes ; $i++) {
						...
						if (isset($imglinesize['width']) && isset($imglinesize['height']) && empty($this->picture_after)) {
							if (($curY + $imglinesize['height'] + $ht_url) > ($this->page_hauteur - ($heightforfooter)))	// If photo too high, we moved completely on new page
							{
								$pdf->AddPage('', '', true);
		...
		********************************************/
		protected function _tableau(&$pdf, $object, $tab_top, $tab_height, $outputlangs, $hidetop = 0, $hidebottom = 0, $pagenb)
		{
			...
			elseif ($this->showtblline)	$pdf->line($this->posxcol2, $tab_top, $this->posxcol2, $tab_top + $tab_height, $this->tblLineStyle);
			// En-tête tableau
			$pdf->SetFont('', 'B', $default_font_size - 1);
