<?php
	...
	************************************************/
	...
	/************************************************
	* 	\file		../infraspackplus/core/modules/supplier_order/pdf/pdf_InfraSPlus_CF.modules.php
	* 	\ingroup	InfraS
	* 	\brief		Class file for InfraS PDF suppliers orders
	************************************************/
	...
	/************************************************
	 *	Class to generate PDF supplier order InfraS
	************************************************/
	class pdf_InfraSPlus_CF extends ModelePDFSuppliersOrders
	{
		...
		********************************************/
		public function __construct($db)
		{
			...
			$this->name							= $langs->trans('PDFInfraSPlusSupplierOrderName');
			$this->description					= $langs->trans('PDFInfraSPlusSupplierOrderDescription');
			$this->titlekey						= 'SupplierOrder';
			$this->defaulttemplate				= isset($conf->global->COMMANDE_SUPPLIER_ADDON_PDF)															? $conf->global->COMMANDE_SUPPLIER_ADDON_PDF					: '';
			...
			$this->show_creation_date			= isset($conf->global->INFRASPLUS_PDF_CF_SHOW_CREATION_DATE)												? $conf->global->INFRASPLUS_PDF_CF_SHOW_CREATION_DATE			: 0;
			...
			$this->show_ExtraFieldsLines		= isset($conf->global->INFRASPLUS_PDF_EXFL_CF)																? $conf->global->INFRASPLUS_PDF_EXFL_CF							: 0;
		...
		********************************************/
		public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
		{
			...
			$filesufixe						= ! $this->multi_files || ($this->defaulttemplate && $this->defaulttemplate == 'InfraSPlus_CF') ? '' : '_CF';
					...
					for ($i = 0 ; $i < $nblignes ; $i++) {
						...
						if (!empty($this->with_picture)) {
							...
							if (!empty($this->old_path_photo)) {
								...
								$pdir[0] = get_exdir(0, 0, 0, 0, $objphoto, 'product').((float) DOL_VERSION >= 13.0 ? '' : dol_sanitizeFileName($objphoto->ref).'/');	// default
		...
		********************************************/
		protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
		{
			...
			if (empty($this->header_after_addr)) {
				...
				$posy								+= $this->tab_hl;
				if (!empty($this->show_creation_date) && !empty($object->date)) {
					$txt_datec	= $outputlangs->transnoentities('PDFInfraSPlusCFDtCrea').' : '.dol_print_date($object->date, 'day', false, $outputlangs, true);
