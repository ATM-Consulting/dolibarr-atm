...
if ($id) {
	...
	if ($tabname[$id]) {
		...
		foreach ($fieldlist as $field => $value) {
			...
			if ($value == 'type') {
				if ($tabname[$id] == MAIN_DB_PREFIX."c_paiement") {
					$valuetoshow = $form->textwithtooltip($langs->trans("Type"), $langs->trans((!empty($conf->global->INFRASPLUS_PDF_USE_PAY_SPEC) ? "InfraSPlusDictListPaiements" : "TypePaymentDesc")), 2, 1, img_help(1, ''));	// InfraS change '"TypePaymentDesc"' to '(!empty($conf->global->INFRASPLUS_PDF_USE_PAY_SPEC) ? "InfraSPlusDictListPaiements" : "TypePaymentDesc")'
				} else {
		...
		if (empty($reshook)) {	// // InfraS change
