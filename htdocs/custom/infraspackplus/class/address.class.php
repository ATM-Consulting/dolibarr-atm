<?php
	...
	************************************************/
	...
	class Address
	{
		...
		************************************************/
		function update($id, $socid, $user = '')
		{
			...
			$this->email		= trim($this->email); // InfraS ajout
			$this->url			= trim($this->url); // InfraS ajout
			...
			if ($result >= 0) {
				...
				$sql	.= $user ? ', fk_user_modif = "'.$user->id.'"' : '';
		...
		************************************************/
		function fetch_lines($socid, $user = null)
		{
			...
			$sql		= 'SELECT rowid, nom AS name, client, fournisseur FROM '.MAIN_DB_PREFIX.'societe WHERE rowid = '.$socid;
			...
			if ($resqlsoc) {
				...
				$this->lines	= array();
				if ($this->socid) {	// Adresses liees a la societe
					$sql	= 'SELECT a.rowid AS id, a.label, a.name, a.address, a.datec AS date_creation, a.tms AS date_modification, a.fk_soc';
					$sql	.= ', a.zip, a.town, a.note, a.fk_pays AS country_id, a.phone, a.fax, a.email, a.url';
		...
		************************************************/
		function fetch_address($id, $user = null)
		{
			...
			$sql	.= ', a.zip, a.town, a.note, a.fk_pays as country_id, a.phone, a.email, a.url, a.fax';
			$sql	.= ', c.code as country_code, c.label as country';
			$sql	.= ' FROM '.MAIN_DB_PREFIX.'societe_address as a';
			$sql	.= ' LEFT JOIN '.MAIN_DB_PREFIX.'c_country as c ON a.fk_pays = c.rowid';
		...
		************************************************/
		function delete($id, $socid)
		{
			...
			$sql			= 'DELETE FROM '.MAIN_DB_PREFIX.'societe_address WHERE rowid = '.$id.' AND fk_soc = '.$socid;
			$result			= $this->db->query($sql);
			if (!$result)	return -1;
			return 1;
		}
		...
		************************************************/
		function getNomUrl($withpicto = 0, $option = '')
		{
			...
			$link			= '<a href = "'.DOL_URL_ROOT.dol_buildpath('infraspackplus', 1).'/comm/address.php?id='.$this->id.'&socid='.$this->socid.'" title="'.dol_escape_htmltag($label, 1).'" class = "classfortooltip">';
			$linkend		= '</a>';
	...
	class AddressLine
	{
		...
		public $label;	// @var string stock movements label
		public $name;
		public $address;	// @var string Address
		public $zip;
		public $town;
