...
$result=restrictedArea($user,'produit&associe');
...
elseif(!isset($_REQUEST['button_search_x']) && isset($conf->global->ASS_DEFAUT_FILTER) && $conf->global->ASS_DEFAUT_FILTER >= 0) $sql .= ' AND p.finished = '.$conf->global->ASS_DEFAUT_FILTER;
...
if(!$conf->global->ASS_USE_DELIVERY_TIME) $sql .= $db->plimit($limit + 1, $offset);
$resql = $db->query($sql);
...
if ($resql || $resql2) {
    ...
    $head = array();
    $head[0][0] = dol_buildpath('/associe/cbn.php?id='.$_REQUEST['id'],2);
    $head[0][1] = $title;
    $head[0][2] = 'associe';
           ...
           if(!$justOFforNeededProduct) {
                ...
                if($week_to_replenish>0) {
    				...
    				if(!$conf->global->STOCK_CALCULATE_ON_SUPPLIER_VALIDATE_ORDER || $conf->global->ASS_USE_VIRTUAL_ORDER_STOCK) {
    	                $result=$prod->load_stats_commande_associe(0, '3,4');
    	                if ($result < 0) {
    	                    dol_print_error($db,$prod->error);
    	                }
						...
						if ($row = $db->fetch_object($resqlQ)) $stock_commande_associe -= $row->qty;
...
*/					$prod->load_stock();
                 ...
                 $champs.='<td align="right">'.
                 $form->select_product_fourn_price($prod->id, 'ass'.$i, 1).
                 '</td>';
				print $champs;
    ...
    $(document).ready(function() {
       ...
       $('span.stock_details').each(function(i, item) {
           ...
           $(this).mouseover(function() {
               ...
               $.ajax({
                  url : "<?php echo dol_buildpath('/associe/script/interface.php',1) ?>"
                  ,data:{
                      get : 'stock-details'
                      ,idprod : prodid
                      ,nbweek : nbweek
