...
function _getAvailability()
{
	...
	if($fk_product)
	{
		$productAssocie= new ProductAssocie($db);
		if($productAssocie->fetch($fk_product)>0)
		{
			if($productAssocie->fetch_product_associe_price($fk_price, 1)>0) //Ignore the math expression when getting the price
			{
				$fk_fourn = $productAssocie->fourn_id;
			}
		}

		$nb_day = (int)TASS::getMinAvailability($fk_product, $qty,1,$fk_fourn);
...
function _stockDetails()
{
	...
	if(empty($conf->global->ASS_USE_ONLY_OF_FOR_NEEDED_PRODUCT)) {
		...
		if(!empty($r)) {
			print '<p>';
			print '<strong>Commande associe</strong>';
			print $r;
			print '</p>';
