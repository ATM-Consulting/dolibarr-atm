        ...
        for ($i = 0; $i < $linecount; $i++) {
            ...
            if(GETPOST('check'.$i, 'alpha') === 'on' && (GETPOST('ass' . $i, 'int') > 0 || GETPOST('ass_free' . $i, 'int') > 0)) { //one line
				...
				//Lignes libres
					...
					$fournid = GETPOST('ass_free'.$i, 'int');
...
//				var_dump($obj,$order);exit;
				...
				if($contact_ship && $conf->global->ASSOCIE_CONTACT_DELIVERY) $order->add_contact($contact_ship, 'SHIPPING');
				$order->add_object_linked('commande', $_REQUEST['id']);
				$newCommande = true;
...
//var_dump($line);
		...
		/*if($newCommande) {
			...
			setEventMessage("Commande associe créée avec succès !", 'errors');
    ...
    if ($box === false) {
		...
		if($ajoutes) {
			foreach($ajoutes as $nomAssocie => $nomProd) {
				$mess.= "Produit ' ".$nomProd." ' ajouté à la commande du associe ' ".$nomAssocie." '<br />";
			}
		}
	    // FIXME: same as $ajoutes.
		if($rates) {
			foreach($rates as $nomAssocie => $nomProd) {
				$mess.= "Quantité insuffisante de ' ".$nomProd." ' pour le associe ' ".$nomAssocie." '<br />";
			}
		}
		if($rates) {
