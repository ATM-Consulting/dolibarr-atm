...
function getDayFromAvailabilityCode($av_code) {
	return TASS::getDayFromAvailabilityCode($av_code);
}
function getMinAvailability($fk_product, $qty) {
	global $db,$form;
	return TASS::getMinAvailability($fk_product, $qty);
}


function _load_stats_commande_associe($fk_product, $date,$stocktobuy=1,$filtrestatut='3') {
    global $conf,$user,$db;
...
function getLinkedAssocie($sourceCommandeId,$supplierSocId,$shippingContactId=0,$status=-1,$array_options=array())
{
    global $db, $conf;
    $shippingContactId = intval($shippingContactId);
    $status = intval($status);
...
function sofo_getFournMinPrice($fk_product)
{
    ...
    $TfournPrices = $ProductAssocie->list_product_fournisseur_price($fk_product, '', '', 1);
...
function associeAdminPrepareHead()
{
    global $langs, $conf;
    ...
    $head[$h][0] = dol_buildpath("/associe/admin/associe_setup.php", 1);
    $head[$h][1] = $langs->trans("Parameters");
    $head[$h][2] = 'settings';
    $h++;
    
    if (!empty($conf->nomenclature->enabled)){
        $head[$h][0] = dol_buildpath("/associe/admin/dispatch_to_supplier_order_setup.php", 1);
    ...
    //$this->tabs = array(
    //	'entity:+tabname:Title:@associe:/associe/mypage.php?id=__ID__'
    //); // to add new tab
    //$this->tabs = array(
    //	'entity:-tabname:Title:@associe:/associe/mypage.php?id=__ID__'
    //); // to remove a tab
    complete_head_from_modules($conf, $langs, $object, $head, $h, 'associeadmin');
