...
dol_include_once('ass/class/associe.product.class.php');
dol_include_once('associe/lib/function.lib.php');

if(empty($user->rights->associe->commande->creer)) accessforbidden();
...
$TfournUnitPrice = GETPOST('assUnitPrice','array');
	...
	if($action == 'dispatch')
	{
        ...
        foreach ($origin->lines as $i => $line)
        {
            ...
            if(!empty($TChecked) && in_array($line->id, $TChecked))
            {
                ...
                // vérification si la ligne fait déjà l'objet d'une commande associe
                ...
                if(empty($searchSupplierOrderLine))
                {
                    ...
                    $searchSupplierOrder = getLinkedAssocie($line->fk_commande,$supplierSocId,$shippingContactId,CommandeFournisseur::STATUS_DRAFT,$array_options);
                    if(empty($searchSupplierOrder))
                    {
                        // search draft supplier order with same critera
                        $restrictToCustomerOrder=0;
                        if(!empty($conf->global->ASS_USE_RESTRICTION_TO_CUSTOMER_ORDER)){
                    ...
                    if(!empty($searchSupplierOrder))
                    {
                        $CommandeAssocie->fetch($searchSupplierOrder[0]);
                    }
                    else
                    {
                        $CommandeAssocie->socid = $supplierSocId;
                        $CommandeAssocie->mode_reglement_id = $societe->mode_reglement_supplier_id;
                        $CommandeAssocie->cond_reglement_id = $societe->cond_reglement_supplier_id;
                        $id = $CommandeAssocie->create($user);
                        if($id>0)
                        {
                            // Add shipping contact
                            if(!empty($TShipping[$line->id])){
                                $CommandeAssocie->add_contact($TShipping[$line->id], 'SHIPPING');
                            }

                            // add order in linked element
                            $CommandeAssocie->add_object_linked('commande', $origin->id);
                    ...
                    if(!empty($line->fk_product)){
                        $ProductAssocie = new ProductFournisseur($db);
                        if($ProductAssocie->find_min_price_product_fournisseur($line->fk_product, $qty, $supplierSocId)>0){
                            $price = floatval($ProductAssocie->fourn_price); // floatval is used to remove non used zero
                            $tva_tx = $ProductAssocie->tva_tx;
                            $fk_prod_fourn_price = $ProductAssocie->product_fourn_price_id;
                            $remise_percent = $ProductAssocie->fourn_remise_percent;
                            $ref_supplier= $ProductAssocie->ref_supplier;
                        }
                    }

                    //récupération du prix d'achat de la line si pas de prix associe
                    ...
                    if(!empty($CommandeAssocie->lines) && $conf->global->ASS_ADD_QUANTITY_RATHER_THAN_CREATE_LINES)
                    {
                        foreach ($CommandeAssocie->lines as $li => $fournLine)
                        {
                            if(
                    ...
                    if($supplierLineRowidExist>0)
                    {
                        ...
                        $updateRes = $CommandeAssocie->updateline(
                            $fournLine->id,
                            $fournLine->desc,
                            $fournLine->subprice,
                            $fournLine->qty + $qty,
                        ...
                        if($updateRes>=0) // yes $CommandeAssocie->updateline can return 0 on success
                        {
                            // add order line in linked element
                            $commandeAssocieLigne = new CommandeFournisseurLigne($db);
                            $commandeAssocieLigne->fetch($addRes);
                            $commandeAssocieLigne->add_object_linked('commandedet', $line->id);

                            // sauvegarde des infos pour l'affichage du resultat
                            $TDispatch[$line->id] = array(
                                'status' => 1,
                                'id' => $CommandeAssocie->id,
                                'msg' => $CommandeAssocie->getNomUrl(1)
                            );
                        }
                        else {
                            // sauvegarde des infos pour l'affichage du resultat
                            $TDispatch[$line->id] = array(
                                'status' => -1,
                                'id' => $CommandeAssocie->id,
                                'msg' => $CommandeAssocie->getNomUrl(1).' '.$langs->trans('ErrorUpdateSupplierLine').' #'.$updateRes.' : '.$commandeAssocieLigne->error
                            );
                        }
                    }
                    else
                    {
                        // ADD LINE
                        $addRes = $CommandeAssocie->addline(
                            $line->desc,
                            $price,
                            $qty,
                            $tva_tx,
                        ...
                        if($addRes>0)
                        {
                            ...
                            $TDispatch[$line->id] = array(
                                'status' => -1,
                                'id' => $CommandeAssocie->id,
                                'msg' => $CommandeAssocie->getNomUrl(1).' '.$langs->trans('ErrorAddSupplierLine').' : '.$commandeAssocieLigne->error
                            );
                        }
                    }
            ...
            if(!empty($TCheckedNomenclature[$line->id]) && is_array($TCheckedNomenclature[$line->id]))
            {
                ...
                foreach($TCheckedNomenclature[$line->id] as $nomenclatureI)
                {
                    ...
                    if(empty($searchSupplierOrderLine))
                    {
                        ...
                        if(!empty($line->fk_product)){
                            $ProductAssocie = new ProductFournisseur($db);
                            if($ProductAssocie->find_min_price_product_fournisseur($product->id, $qty, $supplierSocId)>0){
                                $price = floatval($ProductAssocie->fourn_price); // floatval is used to remove non used zero
                                $tva_tx = $ProductAssocie->tva_tx;
                                $fk_prod_fourn_price = $ProductAssocie->product_fourn_price_id;
                                $remise_percent = $ProductAssocie->fourn_remise_percent;
                        ...
                        //récupération du prix d'achat de la produit si pas de prix associe
                        /*if(empty($price) && !empty($line->pa_ht) ){
                            $price = $line->pa_ht;
                        }*/
...
llxHeader('', $langs->trans('Dispath'), $helpurl, 'commercial', 0, 0, '', array('/associe/css/style.css'));


$head = array();
$head[0][0] = dol_buildpath('/associe/ordercustomer.php?id='.$fromid,1);
$head[0][1] = $title;
$head[0][2] = 'associe';


$head[1][0] = dol_buildpath('/associe/dispatch_to_supplier_order.php',1).'?from='.$from.'&fromid='.$fromid;
$head[1][1] = $langs->trans('ProductsAssetsToOrder');
$head[1][2] = 'associe_dispatch';


dol_fiche_head($head, 'associe_dispatch', $langs->trans('Replenishment'), 0, 'stock');
...
$thisUrlStart = dol_buildpath('associe/dispatch_to_supplier_order.php',1);
...
if( ($action === 'prepare' || $action == 'showdispatchresult')  && !empty($origin->lines)){
    ...
    if(!empty($conf->global->ASS_USE_DELIVERY_CONTACT)){
        $totalNbCols++;
    ...
    if(!empty($origin->lines)){
        ...
        foreach($origin->lines as $i => $line){
            ...
            if(!$line->isModSubtotalLine  && empty($searchSupplierOrderLine))
            {
                ...
                if(empty($conf->global->ASS_DISABLE_ORDER_POSIBILITY_TO_PRODUCT_WITH_NOMENCLATURE)){
                ...
                if(!empty($searchSupplierOrderLine))
                {
                    ...
                    foreach($searchSupplierOrderLine as $searchSupplierOrderLineId)
                    {
                        ...
                        $commandeAssocieLigne = new CommandeFournisseurLigne($db);
                        $resFetchCommandeAssocieLigne = (int) $commandeAssocieLigne->fetch($searchSupplierOrderLineId);
                        if($resFetchCommandeAssocieLigne > 0){

                            // petite opti
                            if(!in_array($commandeAssocieLigne->fk_commande, $TAllreadyDispatchedSuppOrderLines))
                            {

                                $existingFournOrder = New CommandeFournisseur($db);
                                $existingFournOrder->fetch($commandeAssocieLigne->fk_commande);
                            ...
                            $TAllreadyDispatchedSuppOrderLines[$commandeAssocieLigne->id] = $commandeAssocieLigne->fk_commande;
    ...
    print '<script type="text/javascript" src="'.dol_buildpath('associe/js/dispatch_to_supplier_order.js.php',1).'"></script>';
...
function _nomenclatureViewToHtml($line, $TNomenclatureLines, $overrideParam = array())
{
    ...
    foreach ($TNomenclatureLines as $i => $productPart)
    {
        ...
        if($productPart['element'] != 'workstation' && $product )
        {
            ...
            if (!empty($conf->global->ASS_FILL_QTY_NOMENCLATURE)){
