...
$id					= GETPOST('id', 'int');
$statement_number	= GETPOST('statement_number', 'alpha');
    ...
    if ($user->rights->banque->consolidate) {
        if ($id > 0 || $start_date > 0) {
            ...
            if ($result < 0) {
                ...
                $result = $budgetinsight->autoLinkBankRecords($user, $id, $statement_number, $state);
                if (!is_array($result)) {
                    $outjson = array(
                    ...
                    if ($result['status'] == 0) {
                        ...
                        if (!empty($result['text'])){
                            $outjson['text'] = $result['text'];
                            $outjson['status'] = '';
                        }
                        if (!empty($result['require_confirmation'])) {
                            $outjson['location'] = dol_buildpath('/banking4dolibarr/prelink.php', 1) . '?id=' . $id . '&state=' . $result['state'] . (!empty($statement_number) ? '&statement_number=' . $statement_number : '');
                        }
                        if ($result['nb_processed'] > 0) {
                            setEventMessage($langs->trans('Banking4DolibarrAutoReconcileBankRecordsSuccess', $result['nb_processed']));
                            $outjson['keep_window_open'] = true;
                        } else {
                            setEventMessage($langs->trans('Banking4DolibarrAutoReconcileBankRecordsSuccessNoneProcessed'));
                        }
                    } else {
                        $outjson = array(
                        ...
                        if (!empty($statement_number)) $outjson['data']['statement_number'] = $statement_number;
                        if (!empty($result['text'])) $outjson['text'] = $result['text'];
                    }
                }
            } else {
                $langs->load('errors');
