...
$langs->loadLangs(array('bills', 'companies', 'products', 'categories', 'banks', 'compta'));
...
$search_bank = GETPOST('search_bank', 'alpha');
...
$arrayfields = array(
	...
	// Modification OpenDSI - Début
	's.code_client'=>array('label'=>"CustomerCode", 'checked'=>1, 'position'=>51),
	's.code_compta'=>array('label'=>"CustomerAccountancyCode", 'checked'=>1, 'position'=>52),
	's.siren'=>array('label'=>"ProfId1FR", 'checked'=>1, 'position'=>53),
	// Modification OpenDSI - Fin																						  
	...
	'b.label'=>array('label'=>"BankCode", 'checked'=>1, 'position'=>93),
...
$sql .= ' s.rowid as socid, s.nom as name, s.name_alias as alias, s.email, s.phone, s.fax, s.address, s.town, s.zip, s.fk_pays, s.client, s.fournisseur, s.code_client, s.code_fournisseur, s.code_compta as code_compta_client, s.code_compta_fournisseur, s.siren,';
$sql .= ' b.rowid as browid, b.ref as bref, b.label as blabel, b.number as bnumber,';																					 
...
$sql .= ' LEFT JOIN '.MAIN_DB_PREFIX.'bank_account AS b ON f.fk_account = b.rowid';
...
if ($search_code_compta) {
	$sql .= natural_search('s.code_compta', $search_code_compta);
}
if ($search_siren) {
...
if ($search_bank) {
    $sql .= natural_search('b.ref', $search_bank);
}
...
if ($search_date_creation_start) {
	$sql .= " AND f.datec >= '".$db->idate($search_date_creation_start)."'";
}
if ($search_date_creation_end) {
	$sql .= " AND f.datec <= '".$db->idate($search_date_creation_end)."'";
}
// Modification OpenDSI - Fin
if ($option == 'late') {
...
if (!$sall) {
	...
	$sql .= ' s.rowid, s.nom, s.name_alias, s.email, s.phone, s.fax, s.address, s.town, s.zip, s.fk_pays, s.client, s.fournisseur, s.code_client, s.code_fournisseur, s.code_compta, s.code_compta_fournisseur, s.siren,';
	$sql .= ' b.rowid, b.label,';
	$sql .= ' typent.code,';
...
if ($resql) {
	...
	// Modification OpenDSI - Début
	if ($search_code_client) {
		$param .= '&search_code_client='.urlencode($search_code_client);
	}
	if ($search_code_compta) {
		$param .= '&search_code_compta='.urlencode($search_code_compta);
	}
	if ($search_siren) {
		$param .= '&search_siren='.urlencode($search_siren);
	}
	// Modification OpenDSI - Fin
	if ($search_town) {
	...
	if (!empty($arrayfields['s.code_client']['checked'])) {
		print '<td class="liste_titre"><input class="flat maxwidth75imp" type="text" name="search_code_client" value="'.dol_escape_htmltag($search_code_client).'"></td>';
	}
	// Code compta
	if (!empty($arrayfields['s.code_compta']['checked'])) {
		print '<td class="liste_titre"><input class="flat maxwidth75imp" type="text" name="search_code_compta" value="'.dol_escape_htmltag($search_code_compta).'"></td>';
	}
	// Siren
	if (!empty($arrayfields['s.siren']['checked'])) {
		print '<td class="liste_titre"><input class="flat maxwidth75imp" type="text" name="search_siren" value="'.dol_escape_htmltag($search_siren).'"></td>';
	}
	// Modification OpenDSI - Fin
	// Town
	...
	if (!empty($arrayfields['b.label']['checked'])) {
        ...
        print '<input class="flat" type="text" size="4" name="search_bank" value="' . dol_escape_htmltag($search_bank) . '">';
        print '</td>';
    }
	...
	if (!empty($arrayfields['f.datec']['checked'])) {
		...
		print '<div class="nowrap">';
		print $langs->trans('From').' ';
		print $form->selectDate($search_date_creation_start ? $search_date_creation_start : -1, 'search_date_creation_start', 0, 0, 1);
		print '</div>';
		print '<div class="nowrap">';
		print $langs->trans('to').' ';
		print $form->selectDate($search_date_creation_end ? $search_date_creation_end : -1, 'search_date_creation_end', 0, 0, 1);
		print '</div>';
		// Modification OpenDSI - Fin
		print '</td>';
	}
	...
	if (!empty($arrayfields['b.label']['checked'])) {
		print_liste_field_titre($arrayfields['b.label']['label'], $_SERVER['PHP_SELF'], 'b.label', '', $param, 'align="center"', $sortfield, $sortorder);
	}
	if (!empty($arrayfields['f.total_ht']['checked'])) {
	...
	if ($num > 0) {
		...
		while ($i < min($num, $limit)) {
            ...
            $bankaccountstatic->id = $obj->browid;
            $bankaccountstatic->ref = $obj->bref;
            $bankaccountstatic->label = $obj->blabel;
            $bankaccountstatic->number = $obj->bnumber;
