...
class Actionschangetiers
{
    ...
    function printCommonFooter($parameters, &$object, &$action, $hookmanager) {
        ...
        $element_authorized = array('propal', 'commande', 'facture', 'supplier_proposal', 'order_supplier', 'invoice_supplier', 'expedition', 'shipping');
        if (($action == '' || !in_array($action, $actions_exclude)) && (in_array($object->element, $element_authorized) && $conf->global->{strtoupper($object->element).'_CHANGE_THIRDPARTY'})) {
            $jsscript .= '<script>';
