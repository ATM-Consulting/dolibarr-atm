...
$(function(){
    if (typeof changeTiers != 'undefined' && typeof urlConf != 'undefined') {
        ...
        if (window.location.pathname == urlConf && changeTiers == true) {
            $( "<a href='#' id='changetiersbtn'>"+pictoChangeTiers+"</a>" ).insertAfter($('.refidno a[href*="comm/card.php?socid="], .refidno a[href*="fourn/card.php?socid="], .refidno a[href*="societe/card.php?socid="], .refidno a[href*="societe/soc.php?socid="]').last());
	    	//alert($(".refidno a[href*=\"societe/card.php?socid=\"]"));
