...
class DiscountAbsolute
{
	...
	public function create($user)
	{
		...
		$iEntity = $invoice->entity;
		if(empty($iEntity)) $iEntity = $conf->entity;
		// Insert request
		...
		$sql .= ")";
		$sql .= " VALUES (".$iEntity.", '".$this->db->idate($this->datec != '' ? $this->datec : dol_now())."', ".((int) $this->fk_soc).", ".(empty($this->discount_type) ? 0 : intval($this->discount_type)).", ".((int) $userid).", '".$this->db->escape($this->description)."',";
