...
if (in_array($object->element, array('propal', 'commande', 'facture')) && !empty($conf->eurochefprojet->enabled) && $user->rights->eurochefprojet->lire_pv && $object->status == $object::STATUS_VALIDATED) {
	print '<th class="linerefsupplier maxwidth125"><span id="selling_price">'.$langs->trans("EurochefProjetSellingPriceCatalogue").'</span></th>';
}
if (in_array($object->element, array('propal', 'commande', 'facture')) && !empty($conf->eurochefprojet->enabled) && $user->rights->eurochefprojet->lire_pa && $object->status == $object::STATUS_VALIDATED) {
	print '<th class="linerefsupplier maxwidth125"><span id="purchase_price">'.$langs->trans("EurochefProjetPurchasePriceCatalogue").'</span></th>';
}

// VAT
print '<th class="linecolvat right" style="width: 80px">';
...
// Price HT
print '<th class="linecoluht right" style="width: 80px">'.$langs->trans('PriceUHT').'</th>';
...
if ($inputalsopricewithtax) {
	print '<th class="right" style="width: 80px">'.$langs->trans('PriceUTTC').'</th>';
}
...
// Reduction short
print '<th class="linecoldiscount right">'.$langs->trans('ReductionShort').'</th>';
