 ...
 */
class pdf_carafacpiso extends ModelePDFFactures
{
     /**
	...
	public function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "carafacpiso";
		$this->description = $langs->trans('iso précaire');
	...
	protected function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs)
	{
		...
		if (($deja_regle > 0 || $creditnoteamount > 0 || $depositsamount > 0) && empty($conf->global->INVOICE_NO_PAYMENT_DETAILS))
		{
			...
			foreach($object->array_options as $key_option=>$options){
				$matches='';
				preg_match('/_(prime.)/', $key_option, $matches);
				//print $matches;
				if (count($matches)>0 && $options!='0')
				{
					$lib_option=$extrafields->showOutputField($matches[1], $options);
					
					$index++;
					$pdf->SetFillColor(255, 255, 255);
		
					$pdf->SetXY($col1x, $tab2_top + $tab2_hl * $index);
					$pdf->MultiCell($col2x - $col1x, $tab2_hl, $outputlangs->transnoentities($lib_option), $useborder, 'L', 1);
					$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
					$prime=$object->array_options['options_mt'.$matches[1]];
					$pdf->MultiCell($largcol2, $tab2_hl, price($prime, 0, $outputlangs), $useborder, 'R', 1);
					$sum_prime+=$prime;
				}
			}
			
			$resteapayer=$totalht-$sum_prime+$totaltva-$deja_regle + $depositsamount;
			$index++;
			$pdf->SetTextColor(0, 0, 60);
			$pdf->SetFillColor(224, 224, 224);
			$pdf->SetXY($col1x, $tab2_top + $tab2_hl * $index);
