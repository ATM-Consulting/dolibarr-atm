 ...
 */
class pdf_carassfac extends ModelePDFFactures
{
     /**
	...
	protected function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs)
	{
		...
		for ($i = 0; $i < count($object->lines); $i++)
		{
			...
			if ($object->lines[$i]->fk_product > 0){
				...
				if ($product->array_options['options_prime']>0){
					...
					$tab_prime[$j]['label']=$object->lines[$i]->libelle;
		...
		$totalSansPrime=$total_ht+$total_prime;
		$pdf->SetXY($col2x, $tab2_top + 0);
		$pdf->MultiCell($largcol2, $tab2_hl, price($sign * ($total_ht ), 0, $outputlangs).' €', 0, 'R', 1);
		...
		if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT))
		{
			...
			else
			{
				...
				// Total TTC
				//$totalTtcSansPrime=$total_ht+$total_prime+$tvaval;
				$index++;
				$pdf->SetXY($col1x, $tab2_top + $tab2_hl * $index);
				...
				foreach ($tab_prime as $prime){
					...
					$pdf->SetTextColor(0, 0, 60);
					$pdf->SetFillColor(255, 255, 255);
					$pdf->MultiCell($col2x - $col1x+20, $tab2_hl, $prime['label'], $useborder, 'L', 1);
				...
				// Total TTC
				$totalTtcAvecPrime=$total_ht+$total_prime+$totaltva;
				$index++;
				$pdf->SetXY($col1x, $tab2_top + $tab2_hl * $index);
				$pdf->SetTextColor(0, 0, 60);
				$pdf->SetFillColor(224, 224, 224);
				$pdf->MultiCell($col2x - $col1x, $tab2_hl, $outputlangs->transnoentities("Reste à charge"), $useborder, 'L', 1);

				$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
				$pdf->MultiCell($largcol2, $tab2_hl, price($sign * $totalTtcAvecPrime, 0, $outputlangs).' €', $useborder, 'R', 1);
