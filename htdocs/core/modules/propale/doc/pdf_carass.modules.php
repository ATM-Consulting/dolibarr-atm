 ...
 */
class pdf_carass extends ModelePDFPropales
{
	/**
	...
	protected function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs)
	{
		...
		for ($i = 0; $i < count($object->lines); $i++)
		{
			...
			if ($object->lines[$i]->fk_product > 0){
				...
				if ($product->array_options['options_prime']>0 ){ 
					...
					$tab_prime[$j]['id']=$object->lines[$i]->fk_product;
		...
		if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT))
		{
			...
			else
			{
				...
				//Sous total AF
				$stotalhtaf=$total_ht+$tab_prime[0]['amount'];
				$index++;
				$pdf->SetXY($col1x, $tab2_top + $tab2_hl * $index);
				$pdf->SetTextColor(0, 0, 60);
				$pdf->SetFillColor(224, 224, 224);
				$pdf->MultiCell($col2x - $col1x, $tab2_hl, $outputlangs->transnoentities("Sous Total HT AF"), $useborder, 'L', 1);

				$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
				$pdf->MultiCell($largcol2, $tab2_hl, price($stotalhtaf, 0, $outputlangs), $useborder, 'R', 1);

				$total_resteap=$stotalhtaf+$object->total_tva;
				$index++;
				$pdf->SetTextColor(0, 0, 60);
				$pdf->SetFillColor(224, 224, 224);
				$pdf->SetXY($col1x, $tab2_top + $tab2_hl * $index);
				...
				$pdf->MultiCell($largcol2, $tab2_hl, number_format($total_resteap,2,',',' '), $useborder, 'R', 1);
