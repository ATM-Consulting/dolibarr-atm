 ...
 */
class pdf_caraprop extends ModelePDFPropales
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->posref=$this->marge_gauche + 1;
...
//autres logos
		...
		if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT))
		{
			...
			else
			{
				...
				foreach ($object->array_options as $label=>$prime){
					...
					if( ($prime != null && $prime != 0) && substr($label,8,2) !='mt'){
				...
				$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
				$sign=-1;
				$pdf->MultiCell($largcol2, $tab2_hl, price($total_ttc-$totaladeduire, 0, $outputlangs), $useborder, 'R', 1);
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		if ($showaddress)
		{
			...
			$this->setzipthirdparty($object->thirdparty);
	...
	function setzipthirdparty(&$thirdparty){
		$sql='Select  cp, b.ville from llx_societe_extrafields a, llx_cara_cpville b';
		$sql.= ' where fk_object='.$thirdparty->id;
		$sql.= ' and a.ville=b.rowid';
		$resql = $this->db->query($sql);
		if ($resql) {
			$obj = $this->db->fetch_object($resql);
			$thirdparty->zip=$obj->cp;
			$thirdparty->town=$obj->ville;
		}
