...
// Code Compte header
        // Largeur souhaitée pour la colonne "Code Compta", par exemple 30 au lieu de 40.
        $codeComptaWidth = 20;
        $pdf->SetXY(81, $this->tab_top + 2);
        $pdf->MultiCell($codeComptaWidth, 2, $outputlangs->transnoentities("Code Compta"), 0, 'L');

// Ajuster la position de la ligne verticale en fonction de la nouvelle largeur.
        $pdf->line(81 + $codeComptaWidth, $this->tab_top, 81 + $codeComptaWidth, $this->tab_top + $this->tab_height + 10);

// CheckTransmitter header
        $pdf->SetXY(121, $this->tab_top + 2);
...
// Amount header
        $pdf->SetXY(161, $this->tab_top + 2);
        $pdf->MultiCell(40, 2, $outputlangs->transnoentities("Amount"), 0, 'R');
    ...
    public function Body(&$pdf, $pagenb, $pages, $outputlangs)
    {
        ...
        for ($j = 0; $j < $num; $j++) {
            ...
            $pdf->MultiCell(50, $this->line_height, dol_trunc($outputlangs->convToOutputCharset($this->lines[$j]->bank_chq), 44), 0, 'L', 0);

            // Code Compte cell
            $pdf->SetXY(80, $this->tab_top + 10 + $yp);
            $pdf->MultiCell(40, $this->line_height, dol_trunc($outputlangs->convToOutputCharset($this->lines[$j]->code_compta), 24), 0, 'L', 0);  // Assuming the field in your object is named code_compte

            $pdf->SetXY(103, $this->tab_top + 10 + $yp);
            $pdf->MultiCell(50, $this->line_height, dol_trunc($outputlangs->convToOutputCharset($this->lines[$j]->emetteur_chq), 28), 0, 'L', 0);
