...
class Societe extends CommonObject
{
	...
	public $fields = array(
		...
		'note_private' =>array('type'=>'text', 'label'=>'NotePrivate', 'enabled'=>1, 'visible'=>0, 'position'=>225),
		'note_public' =>array('type'=>'text', 'label'=>'NotePublic', 'enabled'=>1, 'visible'=>0, 'position'=>230),
	...
	public function update($id, $user = '', $call_trigger = 1, $allowmodcodeclient = 0, $allowmodcodefournisseur = 0, $action = 'update', $nosyncmember = 1)
	{
		...
		if ($result >= 0) {
			...
			if ($resql) {
				...
				// update accountancy for this entity
				if (!$error && !empty($conf->global->MAIN_COMPANY_PERENTITY_SHARED)) {
					$this->db->query("UPDATE " . MAIN_DB_PREFIX . "societe SET accountancy_code_sell = '', accountancy_code_buy = '' WHERE fk_soc = " . $this->id . " AND entity = " . $conf->entity);
