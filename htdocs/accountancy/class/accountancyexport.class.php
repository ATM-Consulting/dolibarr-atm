...
class AccountancyExport
{
	...
	public function exportWinfic($objectLines, $exportFile = null)
	{
		...
		foreach ($objectLines as $line) {
			...
			if ($line->sens == 'D') {
				...
				$tab['montant_credit'] = str_pad(number_format(0, 2, ',', ''), 13, ' ', STR_PAD_LEFT);
			} else {
				$tab['montant_debit']  = str_pad(number_format(0, 2, ',', ''), 13, ' ', STR_PAD_LEFT);

				$tab['montant_credit'] = str_pad(number_format($line->credit, 2, ',', ''), 13, ' ', STR_PAD_LEFT);
			}
	...
	/**
	 * Export format : FEC3 (Spécifique réseau Eurochef)
	 *
	 * @param 	array 		$objectLines 			data
	 * @param 	resource	$exportFile				[=null] File resource to export or print if null
	 * @return	void
