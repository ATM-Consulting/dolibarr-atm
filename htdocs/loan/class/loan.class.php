...
class Loan extends CommonObject implements \JsonSerializable
{
	 ...
	 *  @return int           success/error: <0 if KO, id if OK
	 */
	 ...
	 *  @return int         success/error: <0 if KO, >0 if OK
	 */
	public function delete($user, $notrigger = false)
	{
		...
		if (! $error)
		{
			...
			// no need to delete the loan_schedule elements (the foreign key has ON DELETE CASCADE)
			dol_syslog(get_class($this). '::delete', LOG_DEBUG);
			$resql=$this->db->query($sql);
			if (! $resql)
			{
	 ...
	 *  @return int         success/error: <0 if error, >0 if ok
	 */
