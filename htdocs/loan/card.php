...
if ($id > 0)
{
	...
	if ($result > 0)
	{
		...
		if ($action == 'edit')
		{
			...
			print '<input name="capital" size="10" value="' . price($object->capital) . '"></td></tr>';
			print '</td></tr>';
		}
		else
		{
		...
		if ($resql)
		{
			...
			while ($i < $num)
			{
				...
				print '<td class="right">'.price($objp->amount_insurance, 0, '', 1, -1, -1, $conf->currency)."</td>\n";
				print '<td class="right">'.price($objp->amount_interest, 0, '', 1, -1, -1, $conf->currency)."</td>\n";
				print '<td class="right">'.price($objp->amount_capital, 0, '', 1, -1, -1, $conf->currency)."</td>\n";
				print '</tr>';
			...
			if ($object->paid == 0)
			{
				print '<tr><td colspan="5" class="right">'.$langs->trans('AlreadyPaid').' :</td><td class="right">'.price($totalpaid, 0, '', 0, 0, -1, $conf->currency).'</td></tr>';
				print '<tr><td colspan="5" class="right">'.$langs->trans('AmountExpected').' :</td><td class="right">'.price($object->capital, 0, '', 1, -1, -1, $conf->currency).'</td></tr>';
		...
		{
			print '<div class="center">';
			print '<input type="submit" accesskey="s" class="button" name="save" value="'.$langs->trans('Save').'">';
			print '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
			print '<input type="submit" accesskey="z" class="button" name="cancel" value="'.$langs->trans('Cancel').'">';
			print '</div>';
		...
		if ($action != 'edit')
		{
 			...
 			if (empty($reshook))
			{
				...
				{
					print '<div class="inline-block divButAction"><a class="butAction" accesskey="e" href="'.DOL_URL_ROOT.'/loan/card.php?id='.$object->id.'&amp;action=edit">'.$langs->trans('Modify').'</a></div>';
				}
